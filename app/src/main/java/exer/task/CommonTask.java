...
public class CommonTask extends AsyncTask<String, Integer, String> {
    ...
    private String getRemoteData() {
        ...
        try {
            ...
            connection.setDoInput(true);  // 允許連結 輸入
            connection.setDoOutput(true); // 允許連結 輸出
            /* 不知道請求內容大小時可以呼叫此方法將請求內容分段傳輸，設定0代表使用預設大小
             * 如果不設大小，在傳輸大型檔案時系統將會崩潰 */
            // 參考 HttpURLConnection API 的 Posting Content 部分
            connection.setChunkedStreamingMode(0);
            /* 設定不使用手機快取，總是從資料庫那裡取得最新的資料以免錯誤 */
